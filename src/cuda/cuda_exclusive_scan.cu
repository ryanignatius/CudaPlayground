...
__global__ void scan(int n, int *d_in, int *d_out, int *d_temp){
    ...
    while (step > 0){
        int ileft = id - step;
        if (id % md == md - 1){
            int cur = sdata[ileft] + sdata[id];
            sdata[ileft] = sdata[id];
            sdata[id] = cur;
        }
        __syncthreads();
        step /= 2;
    ...
    if (id == m-1){
        d_temp[blockIdx.x] = sdata[id] + d_in[index];
    }
    __syncthreads();
...
__global__ void add(int n, int *d_in, int *d_out, int *d_temp){
    ...
    if (id == 0){
        ...
        d_out[index] = d_in[index] + d_temp[id];
    }
}
...
int main(){
    ...
    for (int i=1; i<smallN; i++){
        std::cout << x[i] << ": " << xout[i] << " " << smallArr[i-1] << std::endl;
    }
    for (int i=1; i<largeN; i*=10){
        std::cout << i << ": " << yout[i] << " " << largeArr[i-1] << std::endl;
    }
    std::cout << "Scan: " << yout[largeN-1] << " " << largeArr[largeN-2] << std::endl;
